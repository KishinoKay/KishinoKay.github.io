...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const workDetails = {
        ...
        shikou: `
            ...
            <ul>
                <li><strong>プロジェクト概要</strong><br>
                    大学で学んだAI・機械学習の知識を活かし、ユーザーの嗜好とデータベース内の特定個人の嗜好データ群との一致度を予測・可視化するデスクトップアプリを開発しました。本アプリは、ユーザーの入力データと特定の個人が持つ多数の嗜好データを比較し、k-近傍法（k-NN）を用いて総合的な嗜好一致度を算出します。
                </li>
                <li><strong>開発概要</strong><br>
                    開発期間：1週間<br>
                    チーム規模：個人開発<br>
                    開発環境：Python, Flet, SQLite<br>
                    ソースコード: <a href="https://github.com/Ryukoku-Horizon/2024-BasicGroup-SpringApp-Y230193" target="_blank" rel="noopener noreferrer">GitHub</a>
                </li>
                <li><strong>担当箇所</strong><br>
                    個人開発のため、企画、設計、アルゴリズム実装、UIデザイン、テストまでの全工程を担当しました。
                </li>
                <li><strong>技術的なポイント</strong><br>
                    ・機械学習アルゴリズム「k-近傍法（k-NN）」を応用し、嗜好一致度を算出<br>
                    ・データ特性を考慮し、ユークリッド距離に加え、色相の計算には独自に「円環距離」を導入<br>
                    ・Flet（Python製UIフレームワーク）によるクロスプラットフォーム対応<br>
                    ・SQLiteによるデータ永続化と、公平な比較のためのデータ正規化処理
                </li>
                <li><strong>こだわりポイント</strong><br>
                    ・<strong>データ特性に合わせたアルゴリズムの改良</strong>：色相のような循環するデータを正しく扱うため、標準的な手法に「円環距離」の概念を加える独自の工夫を施しました。これにより、より現実に即した精度の高い類似度計算を実現しました。<br>
                    ・<strong>学術知識の実践力</strong>：大学で学んだAI理論を、実際に動作するアプリケーションとして実装し、理論と実践を結びつけました。<br>
                    ・<strong>自主的な学習姿勢</strong>：日本語情報が少ないFletの公式ドキュメントを読み解き、仕様を理解しながら開発を進めました。
                </li>
            </ul>
            <div class="work-links">
